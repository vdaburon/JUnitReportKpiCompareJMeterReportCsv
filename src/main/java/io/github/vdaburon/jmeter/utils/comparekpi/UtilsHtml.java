...
public class UtilsHtml {
    ...
    public static void saveHtmlFile(GlobalResult globalResult, String htmlFileOut, int typeTemplate) throws IOException, TemplateException {
        ...
        if (typeTemplate == JUnitReportCompareJMReportCsv.K_TYPE_HTML_TEMPLATE) {
            template = cfg.getTemplate(JUnitReportCompareJMReportCsv.K_FREEMARKER_HTML_TEMPLATE);
        }

        if (typeTemplate == JUnitReportCompareJMReportCsv.K_TYPE_DIV_HTML_TEMPLATE) {
            template = cfg.getTemplate(JUnitReportCompareJMReportCsv.K_FREEMARKER_DIV_HTML_TEMPLATE);
        }
